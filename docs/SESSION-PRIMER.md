...
| Reality Audit | 🟡 15/16 domini, 47 bug aperti + 1 deferred | 6✅ + 7⚠️ (canali + memoria/RAG + sicurezza + skills/MCP + contatti/profili + automazioni + workflow) + 1🔧 cognition + 1📝 distribuzione (#67 Windows native installer deferred post-v1.0); 1 dominio ❓ rimanente (Osservabilità; Mobile+Condivisione non-core per v1.0). Bug: 5 canali (#10-#14) + 9 memoria/RAG (#15-#18+#25-#29) + 9 sicurezza (#30-#38) + 14 skills/MCP/contatti (#39-#56) + 10 automazioni/workflow/heartbeat (#57-#66, **1🔴**+6🟡+3🟢) + 1 decision (#67 📝). **5 🔴 totali invariati** (#10, #11, #18, #26, #57) — Sprint 8 era release eng, non audit. **Bug #60 mitigato client-side Sprint 7** via ResultBlock redact. FP cumulativo: Sprint 3:1 + Sprint 4:2 + Sprint 5:8 + Sprint 6:2 = **13 FP totali**. **ISO-3 cross-subsystem table chiusa** a 4✅+2⚠️+2❌ |
| Strategy roadmap | ✅ Fase 1+2 done, APP-2 ✅, INST-1..4 ✅ | Hardening + Apertura + Installer Nativi completati. APP-2 mobile ✅ Sprint 7, INST-1/3/4 ✅ + INST-2 rescopato WSL Sprint 8 |
| Production roadmap | 🚚 Sprint 1+2+3+4+5+6+7+8 ✅, Sprint 9 next | 2/10 sprint rimanenti per v1.0 |
| Production blocker | ✅ risolto (Sprint 8) | .deb + .rpm + .dmg + Homebrew scaffolded, WSL path documentato per Windows. Sprint 8 ha chiuso il blocker "distribuzione" |
...
| Installers | ✅ scaffolded, Apple cert setup pending | `packaging/{linux,macos,brew,windows}/` + `.github/workflows/release.yml`. Smoke test locale OK. Maintainer task residuo: configurare 6 GitHub Secrets Apple per abilitare signed+notarized macOS path (vedi `docs/INSTALLER-SIGNING-SETUP.md`) |
