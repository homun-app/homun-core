...
| Asse | Stato | Note |
|---|---|---|
| Codebase | ✅ stabile | 982 test (+34 Sprint 9 OBS-1/2/3 + UPD-1), 0 nuovi clippy warnings, 121K+ LOC Rust |
| Reality Audit | ✅ **16/16 domini**, 47 bug aperti + 1 deferred | 7✅ + 7⚠️ (canali + memoria/RAG + sicurezza + skills/MCP + contatti/profili + automazioni + workflow) + 1🔧 cognition + 1📝 distribuzione (#67 Windows native installer deferred post-v1.0); osservabilità ❓→✅ Sprint 9. Bug: 5 canali (#10-#14) + 9 memoria/RAG (#15-#18+#25-#29) + 9 sicurezza (#30-#38) + 14 skills/MCP/contatti (#39-#56) + 10 automazioni/workflow/heartbeat (#57-#66, **1🔴**+6🟡+3🟢) + 1 decision (#67 📝). **5 🔴 totali invariati**. Bug #64 mitigato passivamente Sprint 9 via metric `homun_heartbeat_last_fire_timestamp` che resta a 0 finché HeartbeatService non viene instanziato — surface l'anomalia in dashboard. FP cumulativo: 13 totali. ISO-3 cross-subsystem table chiusa a 4✅+2⚠️+2❌ |
| Strategy roadmap | ✅ Fase 1+2 done, APP-2 ✅, INST-1..4 ✅, OBS-1/2/3 + UPD-1 ✅ | Hardening + Apertura + Installer + Osservabilità completati. Solo Sprint 10 (release v1.0) rimanente |
| Production roadmap | 🚚 Sprint 1-9 ✅, Sprint 10 next | **1/10 sprint rimanente per v1.0** |
| Production blocker | ✅ risolto | Distribuzione (Sprint 8) + Osservabilità (Sprint 9) entrambi chiusi. Manca solo release engineering finale + smoke test fresh-install |
| Mobile app | ✅ APP-2 done | APP-1 + APP-2 thread-first completati Sprint 7. APP-3 (push, offline) rimandato post-v1.0 |
| Installers | ✅ scaffolded, Apple cert setup pending | `packaging/{linux,macos,brew,windows}/` + `.github/workflows/release.yml`. Maintainer task residui: 6 GitHub Secrets Apple, split repo migration (`docs/MIGRATION-SPLIT-REPO.md`), tap repo + cross-repo PAT |
...
| Split repo decision | ✅ deciso, 🔲 da eseguire | `homun-app/homun` (public, issues+releases) + `homun-app/homun-core` (private, code) — codice prepared in Sprint 9 con `[support] public_repo = "homun-app/homun"` default, migrazione manuale tracciata in `docs/MIGRATION-SPLIT-REPO.md` (~30 min, da eseguire prima di Sprint 10) |
| Last update | 2026-04-15 | Sprint 9 closed |
