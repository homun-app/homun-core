...
73. WS7 built-in template previews — i seed locali `monet/*` dichiarano
    ...
    Follow-up locale/verde: l'import aggiorna il catalogo riusabile; la card
    template apre un dettaglio e `Use template` crea la nuova chat in modalità
...
82d. Evented Automations + Project Access — le automazioni diventano regole
    IFTTT-style `event source -> filter -> action`, mantenendo le schedule
    esistenti come event source time-based. Prima delle regole c'e' la Project
    Access Surface: ogni progetto espone contatti/canali autorizzati e il runtime
    compone contact perimeter, accesso progetto, policy regola, capability policy
    e approval con default fail-closed. Channels, Composio, MCP, skills/addon,
    local computer e scheduler dichiarano trigger/event source nel registry
    unico; quando un provider non supporta push, Homun usa polling schedulato con
    ...
    infine UI builder IF/FILTER/THEN.
