...
| `crates/desktop-gateway/src/gateway_remote_approval.rs` | Marker parsing remote approval, event parts persistiti, actionable card extraction, creazione pending approval, scadenza, controllo codice pending, parsing risposte OK/NO, testo progresso canale, status thread e prompt/input continuation post-approval; non possiede actionable source resolution, execute pending, dispatch remoto, payment approval o browser. |
