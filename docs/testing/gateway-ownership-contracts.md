...
| `crates/desktop-gateway/src/attachments.rs` | Ingestion allegati composer, contesto prompt bounded per attachment persistiti e costruzione user-content multimodale text/image_url per il turno chat; non possiede loop chat, memory recall, prompt packet, vision fallback/preflight o routing agente. |
...
| `crates/desktop-gateway/src/gateway_agent_turn_loop_seed.rs` | Semina iniziale del turno agente: costruisce `LoopState` con prompt packet e messaggi gia' risolti, inizializza i buffer terminali (`memory_answer`, `last_model_error`, `browse_sources`) e resetta il buffer terminale del turno; non possiede stream, round loop, browser executor, tool execution o subagent. |
...
| `crates/desktop-gateway/src/gateway_capability_routing.rs` | Definizioni native workflow/atomic, registry semantico, decisione semantica turn/steering, binding deterministico plugin, forced tool, pruning tool per route e factory del port `GatewayTurnPolicy`; non possiede corpus discovery generico, dispatch tool, payload modello o completion judge. |
...
| `crates/desktop-gateway/src/gateway_turn_trace.rs` | Bootstrap del trace leggibile di ingresso turno (`turn_received`), inclusi opt-out e fallback no-log-dir; non possiede loop agente, budget o decisioni di avanzamento. |
...
| `crates/desktop-gateway/src/gateway_tool_execution.rs` | Dispatch tool chat/browser, effect receipts, lookup del `ValidatedExecutionContract` del turno e confini capability/computer/browse. |
...
| `crates/desktop-gateway/src/gateway_prompt_instructions.rs` | Snippet di istruzioni prompt riutilizzabili, inclusi core operating prompt dell'orchestratore, connected-service tools/servizi scaduti, blocco channel-contact/persona/privacy, destination folders autorizzati per `save_artifact` e relativo rendering del blocco da `ArtifactDestination`, marker progetto `GOAL_PROPOSE`, discovery browser, marker `CHOICES`/`CLARIFY`, scelta booking, contratto operativo del piano (`update_plan`/`step_advance`), contratto memoria/recall/Vault, contratti mode `plan`/`ask`/`debug`, contratto lingua utente, contratto code-map disponibile, contratto verifica esecuzione, contratto freshness, contratto objective, composizione delle istruzioni runtime-control finali e backup legacy choice-resume; non possiede tool schema, stato `runtime_plans`, stall budget, recall service, prompt packet, artifact destination storage/routes, contact/channel context, perimetri, profile binding, debug executor, code graph runtime, browser runtime, HITL resume, UI card rendering, sandbox execution, build/test runner, store objective, policy effetti, capability routing, Composio/MCP runtime, confirmation card, automazioni, allegati o loop agente. |
