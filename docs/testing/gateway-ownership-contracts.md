...
| `crates/desktop-gateway/src/gateway_agent_turn_outcomes.rs` | Helper puri di outcome/checkpoint del turno agente: restore da `LoopCheckpoint` con nuovo input e terminal outcome per image rejection gia' consegnata; non possiede loop agente, stream/fanout o browser execution. |
