...
| `crates/desktop-gateway/src/gateway_model_routing.rs` | Risoluzione provider/modello/API key, factory `ModelRouter`, payload provider, log persistito decisioni routing, compaction visibile al modello, port `GatewayContextCompactor` e policy reasoning. |
