...
| `crates/desktop-gateway/src/gateway_chat_vision_preflight.rs` | Orchestrazione pre-loop degli allegati immagine: decide inline/fallback, delega al ruolo vision o risposta anticipata no-vision; non possiede recovery post-loop image rejection, stream transport, toolset, loop agente, browser o subagent. |
