...
| `crates/desktop-gateway/src/gateway_thread_model_context.rs` | Conversione bounded dei messaggi persistiti in contesto modello (`ChatContextMessage`), con filtro server-side di marker/placeholder e skip del prompt corrente; non possiede visible turn, finalizzazione stream, recall tool o loop agente. |
