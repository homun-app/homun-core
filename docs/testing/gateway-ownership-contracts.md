...
| `crates/desktop-gateway/src/gateway_skill_runtime.rs` | Directory skill condivisa, normalizzazione id, creazione skill, discovery prompt, rendering del blocco prompt `INSTALLED SKILLS`/HomunCoder, caricamento progressive-disclosure/adattamento SKILL.md e schemi `use_skill` / `run_in_sandbox`; non possiede route HTTP skill, seed default a boot, dispatch tool o routing capability. |
