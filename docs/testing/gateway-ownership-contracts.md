...
| `crates/desktop-gateway/src/gateway_prompt_instructions.rs` | Snippet di istruzioni prompt riutilizzabili, inclusi discovery browser, scelta booking, contratto operativo del piano (`update_plan`/`step_advance`) e backup legacy choice-resume; non possiede tool schema, stato `runtime_plans`, stall budget, prompt packet o loop agente. |
