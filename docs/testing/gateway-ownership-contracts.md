...
| `crates/desktop-gateway/src/gateway_agent_turn_tail.rs` | Coda post-loop del turno agente: HITL legacy projection, trace finale, learn memoria, refresh code graph, steering fence, publish outcome e cleanup stream; non possiede stream setup, loop agente, browser o subagent. |
...
| `crates/desktop-gateway/src/gateway_chat_turn_context.rs` | Setup stateful pre-prompt del turno chat: workspace memoria del thread, contesto contact/channel, real-idle activity e policy mode/tool-policy del turno (`agent` default, read-only, autonomous); non possiede prompt, stream, loop agente, browser o subagent. |
