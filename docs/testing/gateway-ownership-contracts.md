...
| `crates/desktop-gateway/src/gateway_prompt_instructions.rs` | Snippet di istruzioni prompt riutilizzabili, inclusi core operating prompt dell'orchestratore, connected-service tools/servizi scaduti, discovery browser, marker `CHOICES`/`CLARIFY`, scelta booking, contratto operativo del piano (`update_plan`/`step_advance`), contratto memoria/recall/Vault, contratti mode `plan`/`ask`/`debug`, contratto lingua utente, contratto code-map disponibile, contratto verifica esecuzione, contratto freshness, contratto objective e backup legacy choice-resume; non possiede tool schema, stato `runtime_plans`, stall budget, recall service, prompt packet, debug executor, code graph runtime, browser runtime, HITL resume, UI card rendering, sandbox execution, build/test runner, store objective, policy effetti, capability routing, Composio/MCP runtime, confirmation card, automazioni, allegati o loop agente. |
