...
| `crates/desktop-gateway/src/gateway_prompt_instructions.rs` | Snippet di istruzioni prompt riutilizzabili, inclusi discovery browser, scelta booking, contratto operativo del piano (`update_plan`/`step_advance`), contratto memoria/recall/Vault, contratti mode `plan`/`ask`/`debug`, contratto lingua utente, contratto code-map disponibile, contratto verifica esecuzione, contratto freshness, contratto objective e backup legacy choice-resume; non possiede tool schema, stato `runtime_plans`, stall budget, recall service, prompt packet, debug executor, code graph runtime, browser runtime, sandbox execution, build/test runner, store objective, policy effetti o loop agente. |
