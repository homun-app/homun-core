...
|---|---|---|
| **Grafo** (entità + relazioni) | **le sinapsi** — COSA è collegato a COSA + il **PERCHÉ** (archi causali) | tabelle `entities`/`relations`, traversata + `graphify` (import/export `graph.json` + viz) |
...
- **Associativa (il grafo)** 🟡 enorme ma **solo codice**, non decisioni/artefatti/piano
...
> connette il codice ma non il *lavoro/il perché*; gli embedding coprono poco; il
