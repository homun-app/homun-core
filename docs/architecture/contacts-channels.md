...
(store SQLite `~/.homun/desktop-gateway.sqlite`). Allineato a
...
- WhatsApp: `runtimes/channel-whatsapp/src/main.rs:120` `forward_inbound` →
  `POST {gateway}/api/channels/whatsapp/inbound` (`main.rs:137`).
- Telegram: `runtimes/channel-telegram/src/main.rs:305` `forward_inbound` →
...
Route registrate in `main.rs:952` (`whatsapp_inbound`) e `main.rs:959`
(`telegram_inbound`), entrambe delegano a **`handle_channel_inbound`**
(`main.rs:26711`). Il flusso reale:

1. **Owner detection + approval control** (`main.rs:26718`–`26786`): se il mittente è
   l'owner (`channel_message_is_from_owner`) e il testo è un reply di controllo
   `OK 7F3` / `NO 7F3` (`parse_approval_reply`, `main.rs:33493`) su un approval pendente
...
2. **Policy di risposta** (`main.rs:26787`–`26818`): prima la policy globale
   ...
   globale.
3. **Recency + dedup** (`main.rs:26836`–`26904`): scarto dei messaggi troppo vecchi
   (finestra `WA_HISTORY_RECENCY_HOURS`, default 48h), watermark per-contatto, e dedup
   idempotente su `{channel}:{message_id}` (`mark_inbound_seen`). Su errore di store:
   **fail-open** (processa) per non perdere un messaggio nuovo.
4. **Memoria + automazioni** (`main.rs:26906`–`26930`): `record_channel_message`
   registra contatto + episodio; **`fire_channel_event_automations`** (`main.rs:10427`)
   ...
   (fire-and-forget).
5. **Turno** (`main.rs:26931`–`27094`): se l'azione è `AutoReply`/`ApproveReply`:
   - `find_or_create_channel_thread` crea/recupera l'UNICO thread per contatto e ne
     fissa il `channel_recipient` con `set_channel_thread_recipient` (`main.rs:26970`).
     Il reply-target preferisce `sender_pn` > `chat` > `sender` (`main.rs:26940`).
   - **`start_visible_conversation_turn`** (`main.rs:27715`) persiste il bubble utente +
     un placeholder assistant `…`, poi pubblica l'evento **`thread.turn_started`**
     (`thread_turn_started_event`, `main.rs:27691`) così l'app mostra subito il turno.
   ...
   - **`run_agent_turn_into_message`** esegue il turno completo dell'agente sul thread
     con tool **read-only**; se la persistenza del placeholder è fallita si **fa
     fail-closed** (niente lavoro invisibile né auto-reply invisibile, `main.rs:27010`).
...
6. **Persona + perimetro nel loop** (vedi sotto): dentro il turno,
   `contact_turn_context` (`main.rs:27144`) carica persona/tono/relazioni e il
   perimetro, e filtra i tool offerti al modello.
7. **Reply best-effort al canale** (`main.rs:27058`–`27093`):
...
In `run_agent_turn_*` (path condiviso con la chat in-app, ~`main.rs:17943`):

- `(contact_ctx, channel_owner) = contact_turn_context(state, thread_id)`
  (`main.rs:27144`): risolve il contatto per `(channel, sender)` via
  `contact_id_by_identity`; restituisce `None` per thread in-app, mittenti sconosciuti
  e la card dell'owner; `channel_owner=true` SOLO se il mittente è l'`is_self`.
- **`channel_owner` rilassa i gate** che proteggono l'utente DAGLI ALTRI: idle clock
  (`main.rs:17964`), e nel browser la safety-gate consente azioni "committing" se il
  mittente è l'owner (`main.rs:20049`), mentre per gli altri ogni azione che
  conferma/invia è bloccata in turni read-only.
- **Filtro tool del perimetro** (`main.rs:19115`–`19132`): `tools_denied` vince, poi
  `tools_allowed` (se non vuoto) restringe. Match **substring** sul nome funzione,
  composto SOPRA la policy read-only del canale.
...
- Persistenza: `project-access.json` in `~/.homun` (`gateway_project_access_path`,
  `main.rs:43978`), lista di `ProjectAccessGrant` (`main.rs:43901`) chiave
  `(workspace_id, contact_reference, channel)`.
- API: `GET /api/workspaces/{id}/access` (`project_access_list`, `main.rs:44280`),
  `.../access/upsert` (`main.rs:44286`), `.../access/remove` (`main.rs:44317`).
- Resolver: **`resolve_project_contact_policy`** (`main.rs:44059`):
  ...
  - Altrimenti cerca il grant per `(contact_reference, channel)`: **assente →
    `authorized=false`** e tutte le capability negate (fail-closed, `main.rs:44083`).
  - Se presente, i `capability_denies` del grant si SOMMANO a `perimeter.tools_denied`
    (`main.rs:44094`).
- Wrapper canale: `channel_project_contact_policy` (`main.rs:10385`) risolve il
  contatto dal messaggio e produce `EffectiveProjectContactPolicy`. **Consumato OGGI
  solo da `fire_channel_event_automations`** (`main.rs:10463`): se
  `!authorized || !can_trigger_automations` la run è negata, registrata e saltata
  (`main.rs:10464`).

### Approval remoti

- Tabella `remote_approvals` (`chat_store.rs:1938`): `approval_id`, `code` unico,
  `tool`, `arguments_json`, `label`, `thread_id`, `status`, `expires_at`, ecc.
- `deliver_remote_approval` (`main.rs:33472`): crea l'approval pendente
...
- Il giro completo: l'app/agente emette un marker di conferma →
  `activate_remote_approvals_from_message` (`main.rs:2026`) → l'utente risponde
...
flowchart TD
  ...
  TH --> VT[start_visible_conversation_turn pubblica thread turn_started]
...
- **La rubrica è read-model di routing, la verità è la memoria** (caposaldo #1, #5).
  ...
  (`chat_store.rs:244`). L'audit esplicito `CHAT_STORE_MEMORY_BOUNDARY_AUDIT`
  (`chat_store.rs:213`) documenta tabella-per-tabella questo confine.
...
- **Continuità thread-scoped**: una conversazione di canale è UN thread per contatto
  ...
  `main.rs:1999`), con fallback derivato dal thread id.
...
### Perimetro contatto (`StoredPerimeter`, `chat_store.rs:196`)
...
**Come il perimetro filtra i tool nel loop** (`main.rs:19115`): dato l'elenco di tool
offerti al modello, per ogni schema si guarda `/function/name`; se un elemento di
`tools_denied` ne è substring → escluso; poi se `tools_allowed` non è vuoto e nessun
suo elemento è substring del nome → escluso. È composto SOPRA la policy read-only del
canale (i tool committing sono già negati ai non-owner).

### Permessi project access (`ProjectAccessGrant`, `main.rs:43901`)
...
Risoluzione (`resolve_project_contact_policy`, `main.rs:44059`):
...
`is_self` è settato sul contatto (colonna `is_self`, `chat_store.rs:2063`;
`contact_type='self'`). `contact_turn_context` ritorna `(None, true)` per l'owner
(`main.rs:27164`): nessun perimetro restrittivo applicato + `channel_owner=true`. Il
resolver di project access fa bypass. La sua identità è unificata in `person:self` nel
grafo (`unify_owner_identity`, `main.rs:27274`).
...
1. **Project access NON gating sull'auto-reply conversazionale.** Oggi
   ...
   `fire_channel_event_automations` (`main.rs:10463`). Il path di risposta
...
2. **`can_use_project_memory` / `can_receive_replies` / `can_receive_artifacts` sono
   definiti ma poco/non applicati nel runtime.** Sono persistiti, risolti e testati
   (`main.rs:44799+`), ma fuori dal test non risultano consumati: la reply usa la
   memoria secondo `memory_scope` del perimetro, non secondo `can_use_project_memory`
   del grant. Capability ancora dichiarativa.
3. **Matching tool per substring.** Il filtro `tools_denied`/`tools_allowed` è un
   `name.contains(...)` (`main.rs:19124`): rischio di over/under-match (es. negare
   `send` colpisce ogni tool che contiene "send"). Non è un registry-aware match
   (tensione col caposaldo #7).
4. **Dedup fail-open su errore di store** (`main.rs:26894`): scelta deliberata (non
   perdere messaggi), ma in caso di errori persistenti può portare a doppie risposte.
5. **Telegram v1 solo chat dirette** (`runtimes/channel-telegram/src/main.rs:299`); i
   gruppi non sono coperti.
6. **Reply best-effort senza retry applicativo**: `channel_send` fallito è solo loggato
   (`main.rs:27085`); la consegna affidabile è demandata al sidecar
   (`forward_inbound` ha retry at-least-once, ma l'outbound del gateway no).
7. **`from` dell'automazione è match per sottostringa** su nome/sender
   (`main.rs:10456`): può collidere tra contatti con nomi simili.
...
- **#5 — Un solo grafo / niente seconda verità.** `CHAT_STORE_MEMORY_BOUNDARY_AUDIT`
  ...
  perché mirrorato nel grafo canonico.
...
- `crates/desktop-gateway/src/main.rs`
  - `handle_channel_inbound` `:26711` — orchestratore inbound (owner/approval, policy,
    dedup, memoria, turno, reply).
  - `whatsapp_inbound` `:26646` / `telegram_inbound` `:26653`; route `:952` / `:959`.
  - `contact_turn_context` `:27144` + `ContactTurnContext` `:27128` — persona +
    perimetro + `channel_owner`.
  - Filtro tool del perimetro nel loop `:19115`.
  - Gate browser owner-aware `:20049`.
  - `start_visible_conversation_turn` `:27715` / `thread_turn_started_event` `:27691`.
  - `parse_channel_thread_id` `:27117`; `app_channel_reply_target` `:1999`;
    `fallback_channel_recipient_from_thread_id` `:2017`.
  - Project access: `resolve_project_contact_policy` `:44059`,
    `channel_project_contact_policy` `:10385`, `ProjectAccessGrant` `:43901`,
    path `:43978`, route `project_access_list/upsert/remove` `:44280`/`:44286`/`:44317`,
    registrazione route `:982`.
  - `fire_channel_event_automations` `:10427` (gate project access `:10463`).
  - Approval: `deliver_remote_approval` `:33472`, `parse_approval_reply` `:33493`,
    `activate_remote_approvals_from_message` `:2026`, `send_message_tool_schema`
    `:33508`, `execute_send_message` `:33531`.
- `crates/desktop-gateway/src/chat_store.rs`
  - Schema: `contacts` `:2057`, `contact_identities` `:2074`, `contact_perimeters`
    `:2091`, `profiles` `:2106`, `contact_channel_profiles` `:2116`,
    `contact_relationships` `:2124`, `remote_approvals` `:1938`. Colonne thread:
    `source` `:2152`, `channel_recipient` `:2159`.
  - `StoredPerimeter` `:196` + `Default` `:291`; `CHAT_STORE_MEMORY_BOUNDARY_AUDIT`
    `:213`.
  - Metodi: `contact_id_by_identity` `:1204`, `contact_handles` `:1295`,
    `perimeter_or_default` `:1581`, `resolve_profile_for` `:1790`, `relationships_for`
    `:1859`.
