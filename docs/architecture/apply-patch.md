...
Data: 2026-07-03. Il tool `apply_patch` applica patch multi-file in **formato Codex**
...
- **Escalation read-only per apply_patch**: oggi sotto `read-only` nega inline con
  messaggio (l'endpoint `run_escalate` non ri-esegue ancora apply_patch). Estenderlo
  come per `write_file`/`edit_file` (marker `tool=apply_patch`, `arguments={input}`) è il
  follow-up naturale.
