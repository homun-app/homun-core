...
8. Dopo crash, una revisione senza outcome viene recuperata secondo lease/receipt;
   ...
   La conversazione proprietaria è l'unica superficie operativa di questa transizione: legge
   `TaskQueueResponse.uncertain_effects`, mostra una card inline separata dalle approval e invia entrambe
   le decisioni a `POST /api/effects/{receipt_ref}/resolve`. Il task runtime resta autorevole ma non è
   una destinazione di navigazione. La UI non rimuove receipt localmente e non possiede un'altra wake
   o un altro resume.
