...
## Build 2 (steering park+resume) — DONE, with documented follow-up residuals (2026-07-24)

Full park+checkpoint+resume implemented and whole-diff-reviewed (READY TO BUILD; 6 lifecycle invariants confirmed, no Critical/Important cross-task regression). The CRITICAL steering fence-hang and the confidence-0.45 threshold are CLOSED; the resume loop applies steering under the resumed run. Non-blocking follow-ups:
- **desktop `turnStreamRecovery.mjs` doesn't know `"parked"`** → a park > ~30min surfaces a spurious client recovery error (gateway still resumes/delivers; thread reload self-corrects). Teach the recovery loop that `parked` is a low-frequency wait state.
- **live-but-broken semantic provider** (persistent invalid JSON) is indistinguishable from an outage → indefinite park with a misleading "Waiting for the model". Escape: cancel / switch provider.
- **I2 ~100ms sliver**: interpretation succeeding in the window between the coordinator's running-run snapshot and its claim lands an `interpreted` row bound to the aborted run — invisible to the due-scan/resumed run but counted at the fence → parked-forever/re-park. Harden by releasing aborted-run `interpreted` rows at park, or having the fence ignore rows bound to non-running runs.
- Coordinator head-of-line blocking on sequential probes; waiting-Activity once-per-turn-ever; cancel_task held-steering publish gap; T5 fallback comment overstates the Runtime/Json exclusion.
