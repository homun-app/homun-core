...
- [ ] **Step 4: Export types and run focused tests**
...
- [ ] **Step 1: Add failing atomicity and exact-revision tests**
...
- [ ] **Step 2: Run the tests and confirm no enqueue exists**
...
- [ ] **Step 3: Implement deterministic enqueue inside the outcome transaction**
...
- [ ] **Step 4: Add migration backfill tests**
...
- [ ] **Step 1: Add failing transition and concurrency tests**
...
- [ ] **Step 3: Implement atomic claim transitions**
...
- [ ] **Step 5: Commit the claim protocol**
...
- [ ] **Step 1: Add failing projector result tests**
...
- [ ] **Step 2: Run focused gateway tests and confirm mismatch**
...
- [ ] **Step 3: Implement the typed result without changing projection ownership**
...
- [ ] **Step 4: Implement a bounded worker**
...
The worker claims `chat_lifecycle`, loads `execution_revision`, validates scope against the task, invokes the projector, and completes or blocks through the claim API. Retryable SQLite contention receives bounded backoff; invariant violations preserve evidence and set a module-owned `ProjectionWorkerHealth` read by the gateway health endpoint. Startup drain returns the invariant error before task workers start.

- [ ] **Step 5: Replace startup scan and notify after production commit**

After `bump_process_generation` and lease recovery, call `drain_at_startup`. Start the worker before task execution. Replace `replay_committed_chat_projections` calls with `projection_worker::notify()`; do not change `ExecutionContract`, `ExecutionOutcome`, or `EffectHost`.

- [ ] **Step 6: Run gateway and task-runtime tests**
...
- [ ] **Step 1: Add failing resolver transaction tests**
...
- [ ] **Step 2: Run focused tests and verify blocked rows remain blocked**
...
- [ ] **Step 3: Requeue inside the existing receipt transaction**
...
- [ ] **Step 4: Remove resolver history replay**
...
- [ ] **Step 1: Add file-backed restart scenarios**
...
- [ ] **Step 2: Add a test-only projection fault interface**

Define a private test hook with explicit phases:

```rust
#[cfg(test)]
enum ProjectionFaultPoint {
    AfterTask,
    AfterRun,
    AfterMessage,
    AfterObjective,
...
The hook returns an injected interruption; production code has no environment-controlled crash switch. Replay after each interruption must converge to one terminal event.

- [ ] **Step 3: Cover uncertain remote outcome convergence**

Use the existing fake channel/effect adapter to assert that interruption after an ambiguous dispatch leaves one uncertain receipt and blocked outbox row. Resolve `Applied` and `NotApplied` separately and assert zero or one further dispatch respectively.
...
- [ ] **Step 5: Commit the crash harness**
...
- [ ] **Step 1: Remove dead production replay code**
...
- [ ] **Step 2: Update the runtime contract**
...
- [ ] **Step 4: Restart and inspect the development version**
...
- [ ] **Step 5: Commit final cleanup and documentation**
