...
- Modify `apps/desktop/src/locales/{en,it,es,fr,de}.ts`: add parity-checked operational labels and errors.
...
fn uncertain_effect_projection_is_bounded_and_metadata_only() {
    let receipt = ExecutionEffectReceipt {
        receipt_ref: "effect:receipt-1".to_string(),
    ...
    let value = serde_json::to_value(uncertain_effect_response(&receipt)).unwrap();

    assert_eq!(value["receipt_ref"], "effect:receipt-1");
...
```bash
cargo test -p desktop-gateway uncertain_effect_projection_is_bounded_and_metadata_only -- --nocapture
```

Expected: compilation fails because `uncertain_effect_response` and its response type do not exist.

- [ ] **Step 3: Implement the bounded DTO and mapper**
...
Load receipts using `uncertain_effect_receipts_for_user(&state.config.user_id)` inside
...
```bash
cargo test -p desktop-gateway uncertain_effect_projection_is_bounded_and_metadata_only -- --nocapture
cargo test -p desktop-gateway task_queue_response_serializes_ui_read_model_for_renderer -- --nocapture
```
...
fn task_queue_scope_retains_only_matching_uncertain_effects() {
    let mut response = task_queue_response(empty_task_queue_snapshot(), &ResourceLimits::default(), vec![
        uncertain_effect_fixture("effect-1", Some("thread-1")),
        uncertain_effect_fixture("effect-2", Some("thread-2")),
    ]).unwrap();

    retain_task_queue_scope(&mut response, &HashSet::new(), "thread-1");
...
Use the existing snapshot/limit fixture names where available; do not introduce a test-only runtime
store or HTTP server for this pure filtering behavior.
...
```bash
cargo test -p desktop-gateway task_queue_scope_retains_only_matching_uncertain_effects -- --nocapture
```

Expected: compilation fails because `retain_task_queue_scope` does not exist.

- [ ] **Step 3: Move the existing thread filtering into one helper**
...
fn retain_task_queue_scope(
    ...
    allowed_execution_ids: &HashSet<String>,
    thread_id: &str,
) {
    response.queued.retain(|item| allowed_execution_ids.contains(&item.execution_id));
    response.active.retain(|item| allowed_execution_ids.contains(&item.execution_id));
    response.blocked.retain(|item| allowed_execution_ids.contains(&item.execution_id));
    response.recent_failures.retain(|item| allowed_execution_ids.contains(&item.execution_id));
    response.waiting_approvals.retain(|item| item.thread_id.as_deref() == Some(thread_id));
    response.uncertain_effects.retain(|item| item.thread_id.as_deref() == Some(thread_id));
}
```

Call it from the existing `task_queue` endpoint after obtaining the thread's execution IDs.

- [ ] **Step 4: Run the gateway test target and verify GREEN**

Run:

```bash
cargo test -p desktop-gateway task_queue_scope_retains_only_matching_uncertain_effects -- --nocapture
cargo test -p desktop-gateway task_queue_response -- --nocapture
```
...
assert.match(coreBridgeSource, /\/api\/effects\/\$\{encodeURIComponent\(receiptRef\)\}\/resolve/);
...
- [ ] **Step 3: Add bridge types and the resolver call**
...
- [ ] **Step 4: Run contract and type checks and verify GREEN**
...
- [ ] **Step 1: Add failing state-flow assertions**
...
assert.match(appSource, /refreshSelectedTaskDetail\(\)/);
...
- [ ] **Step 3: Implement the Workbench view model and state flow**
...
In `App.tsx`, map `snapshot.uncertain_effects` into state, resolving `scopeLabel` from the already
loaded thread list where possible. Track `effectResolutionBusyId` and `effectResolutionError`
...
try {
  ...
  if (effect.threadId) await refreshChatReadModels(effect.threadId);
} catch (error) {
  setEffectResolutionError(error instanceof Error ? error.message : String(error));
} finally {
...
- Modify: `apps/desktop/src/locales/it.ts`
- Modify: `apps/desktop/src/locales/es.ts`
- Modify: `apps/desktop/src/locales/fr.ts`
- Modify: `apps/desktop/src/locales/de.ts`
- Modify: `apps/desktop/scripts/check-ui-contract.mjs`
...
- [ ] **Step 2: Run UI contract and locale parity tests and verify RED**
...
- [ ] **Step 3: Render the distinct operational section**
...
>
  {busy ? <Loader2 aria-hidden="true" /> : <BadgeCheck aria-hidden="true" />}
...
- [ ] **Step 4: Add complete locale parity**
...
- [ ] **Step 6: Run desktop tests and build and verify GREEN**
...
git add apps/desktop/src/lib/coreBridge.ts apps/desktop/src/types.ts apps/desktop/src/App.tsx \
  apps/desktop/src/components/TasksView.tsx apps/desktop/src/styles.css \
  apps/desktop/src/locales/en.ts apps/desktop/src/locales/it.ts \
  apps/desktop/src/locales/es.ts apps/desktop/src/locales/fr.ts \
  apps/desktop/src/locales/de.ts apps/desktop/scripts/check-ui-contract.mjs
...
- [ ] **Step 1: Run format and warning gates**

```bash
cargo fmt --all -- --check
cargo clippy -p desktop-gateway --all-targets -- -D warnings
npm --prefix apps/desktop run typecheck
```

Expected: all commands exit 0 with no warnings.

- [ ] **Step 2: Run focused and full regression gates**

```bash
cargo test -p desktop-gateway
cargo test -p task-runtime
npm --prefix apps/desktop run test:ui-contract
npm --prefix apps/desktop run test:electron
npm --prefix apps/desktop run build
```
...
- [ ] **Step 3: Verify the dev runtime owns the expected ports**
...
- [ ] **Step 5: Commit plan and verification documentation**
...
- [ ] **Step 6: Confirm clean scope and publish main**
