...
- 🟡 **7.1a** Portare documenti e presentazioni sullo stesso design system:
  ...
  **Follow-up WS7 deck QA metadata (2026-06-24, locale/verde):**
  ...
  e interrogabili anche dopo il turno; il provenance context artifact espone
  `quality: warning` e i codici issue. Test mirati:
...
- ✅ **WS7 real PPTX template import — prima slice locale/verde (2026-06-25):**
  ...
  locale/verde: l'import manuale aggiorna solo il catalogo riusabile; ogni card
  template ha dettaglio e CTA `Use template` che apre una nuova chat in modalità
  piano. Il prompt operativo contiene `template_ref` e vieta la generazione
  immediata, chiedendo prima 2-4 informazioni essenziali e attendendo conferma
  prima di chiamare `make_deck`. Per i template importati, il gateway espone un
...
- 🟡 **9.2 Registry/catalogo sul sito Homun**: **contratto indice JSON fatto** in
  ...
  scrittura atomica. 2026-06-24: aggiunto fetch HTTPS backend
  `POST /api/plugins/registry/fetch`, con limite dimensione e salvataggio nella
  stessa cache. Restano: pubblicare il feed sul sito Homun, packaging `.hplugin`
  e integrazione UI/update.
- 🟡 **9.3 Plugin manager in-app**: prima slice policy fatta nel contratto
  ...
  versioni semver (`is_newer_than`). Verifica 2026-06-24:
  `plugin_registry_entry_applies_install_and_update_policy` e suite capabilities
  verdi. 2026-06-24: aggiunto install manager locale nel gateway:
  `install_hplugin_package()` applica la policy registry, ispeziona/stagea
  `.hplugin`, controlla identità/versione pacchetto e attiva con rename atomico.
  Verifica: `plugin_packages::tests::hplugin_package_install_*` verdi.
  2026-06-24: aggiunto anche endpoint gateway
