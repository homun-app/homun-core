...
pub fn contains_final_payment_action(action: &Value, snapshot: &str) -> bool {
    if is_final_payment_action(action, snapshot) {
        return true;
    }
    action
        .get("actions")
        .and_then(Value::as_array)
        .is_some_and(|actions| actions.iter().any(|nested| contains_final_payment_action(nested, snapshot)))
}

fn is_final_payment_action(action: &Value, snapshot: &str) -> bool {
...
fn is_committing_action(action: &Value) -> bool {
