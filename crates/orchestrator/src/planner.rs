...
pub(crate) fn planner_prompt(
    ...
    let prompt = format!(
        "You are the local-first assistant orchestrator brain.\n\
         ...
         - \"steps\": array of step objects (use [] for direct_answer/ask_clarification/refuse). Each step object MUST have: \"step_id\" (string), \"kind\" (capability_call|memory_lookup|subagent_task|direct_answer), \"depends_on\" (array of step_id), \"execution_policy\" (immediate|durable_task|ask_approval), \"risk_level\" (string), \"expected_duration_seconds\" (integer). A capability_call step adds \"provider_id\",\"tool_name\",\"arguments\". A subagent_task step adds \"agent_id\" (one of the built-in archetypes [PlannerAgent, MemoryAgent, ToolAgent, VisionAgent, RiskAgent, AutomationAgent, ReviewAgent], OR the id of one of the Available specialized agents listed below when one fits the sub-task), plus \"goal\",\"contract\",\"allowed_actions\",\"requires_user_approval\",\"timeout_seconds\",\"max_tokens\".\n\
