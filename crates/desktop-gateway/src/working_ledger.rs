...
pub(crate) fn render(
    ...
    let plan = store.load_runtime_plan(user_id, workspace_id, thread_id)
        .map_err(|error| error.to_string())?;
    let runs = store.list_agent_runs_for_thread(thread_id, user_id, workspace_id)
        .map_err(|error| error.to_string())?;
    let receipts = store.list_tool_receipts_for_thread(thread_id, user_id, workspace_id)
        .map_err(|error| error.to_string())?;
    let mut out = format!("# Working Ledger\n\n- Thread: `{thread_id}`\n- Workspace: `{workspace_id}`\n\n## Runtime plan\n\n");
    if let Some(plan) = plan {
        out.push_str(&format!("Status: `{}` · revision {} · stalled resumes {}\n\n", plan.status, plan.revision, plan.stall_turns));
        if let Some(steps) = plan.plan_json.as_array() {
            for step in steps {
                let status = step.get("status").and_then(|value| value.as_str()).unwrap_or("todo");
                let title = step.get("title").and_then(|value| value.as_str()).unwrap_or("Untitled step");
    ...
    out.push_str("\n## Execution attempts\n\n");
    if runs.is_empty() { out.push_str("No attempts.\n"); }
    for run in &runs {
        out.push_str(&format!("### Attempt {} · `{}`\n\n- Status: `{:?}`\n- Started: `{}`\n- Terminal reason: `{}`\n",
            run.attempt, run.run_id, run.status, run.started_at,
            run.terminal_reason.as_deref().unwrap_or("—")));
        if let Ok(Some(checkpoint)) = store.latest_agent_checkpoint(&run.run_id, user_id, workspace_id) {
            out.push_str(&format!("- Latest safe checkpoint: round {} · `{}`\n", checkpoint.round, checkpoint.fingerprint));
        }
        if let Ok(events) = store.list_agent_run_events(&run.run_id, user_id, workspace_id, None) {
            let timeline = events.into_iter().map(|event| {
                event.round.map(|round| format!("{}@{}", event.kind, round)).unwrap_or(event.kind)
            }).collect::<Vec<_>>().join(" → ");
            if !timeline.is_empty() { out.push_str(&format!("- Timeline: {timeline}\n")); }
        }
        out.push('\n');
    }
    out.push_str("## Effect receipts\n\n");
    if receipts.is_empty() { out.push_str("No effectful tools recorded.\n"); }
