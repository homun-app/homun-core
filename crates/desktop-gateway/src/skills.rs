...
impl SensitiveCategory {
    ...
    /// `pub` because the harness (`main.rs`) re-hydrates categories from the tokens
    /// carried across rounds in the engine's `LoopState` (which is `String`-typed —
    /// the leaf engine crate can't reference this gateway enum).
    ...
    /// The canonical kebab-case token (the round-trip inverse of `parse`). Used to
    /// hand categories to the engine's `String`-typed turn state (`arm_sensitive`).
