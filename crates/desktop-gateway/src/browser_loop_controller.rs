...
- DELIBERATE FALLBACK (not wandering): if the CURRENT source is blocked (captcha/anti-bot) OR clearly has no data matching the criteria after a fair attempt, move to the NEXT source listed in the goal — navigate straight to it. Only switch sites for this reason, never to a random one.
