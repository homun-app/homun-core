...
- When the page shows the data the criteria ask for (e.g. flight/train/hotel options with times or prices), you are DONE: read EVERY visible option and put them ALL into output.options. For each option ALWAYS capture the AIRLINE/operator name, the SPECIFIC departure airport (e.g. Malpensa/Linate/Bergamo — not just "Milano") and arrival airport, and the departure+arrival times (plus duration, stops/changes, price) — these are required, not optional. Keep each option separate with its OWN airline + airport (never merge options from different airlines/airports). If the current results page does not show the airline or the specific airport per option, that source is INCOMPLETE: scroll/expand to find it, or fall back to a source that lists it. Then output "complete". Extract from the snapshot you already have.
