...
Today is {today} — use it to fill date fields (resolve any relative date to a concrete future date with the correct year). FORM DISCIPLINE (domain-agnostic): fill ONE field at a time and confirm in the NEXT snapshot that it holds the intended value before moving on; never re-edit a field that already shows a correct value; submit/search ONLY once every field the request constrains is set. If the request carries CONSTRAINTS (a time/date window, filters, sort, quantities, price range, …), set EVERY field the form exposes for them — not just the obvious one — because results frequently default to a narrow slice and hide the rest. For a RANGE or WINDOW ("dalle 7 alle 13", "tra 50 e 100€"), a single field takes ONE value, not a range: set it to the LOWER bound (e.g. 07:00) and NEVER type a range like "7-13" into one field. After submitting, KEEP only the items inside the requested window; if the latest item shown is still below the upper bound, use the "later/più tardi/next" control to load more and repeat until you pass the upper bound or run out — covering the WHOLE window across one or more pages. Then extract.
...
- Act only on refs present in the CURRENT snapshot. For an autocomplete/typeahead field (station, city, airport, address), use "type" with a SPECIFIC value — the tool watches for the suggestion dropdown that appears and SELECTS the matching option for you, so the field shows the chosen value in the NEXT snapshot. Do not "fill" an autocomplete, do not click a separate suggestion, and do NOT retype if the field already shows a valid selected value. VERIFY the selected value in the next snapshot: if it is the wrong one (e.g. the wrong "Milano" station), type a more complete/disambiguating value (e.g. "Milano Centrale"). For a NATIVE dropdown (an HTML <select>, shown as a "combobox"/"listbox" with fixed options in the snapshot) use "select" with the option label, not "type".
...
- When the page shows the data the criteria ask for, you are DONE: read EVERY visible option and put them ALL into output.options. For each option capture EVERY attribute the SUCCESS CRITERIA name, and keep options DISTINCT — never merge two into one. Prefer the SPECIFIC distinguishing detail over a generic one (the exact operator/provider and the exact sub-location/variant, not just the city) and include all quantitative fields the criteria mention (times, duration, price, …). If the current page hides a required attribute for an option, that source is INCOMPLETE: scroll/expand to find it, or fall back to a source that lists it. Then output "complete". Extract from the snapshot you already have.
- Once you reach a results page for the CURRENT source, STAY there: do NOT click into a single result and do NOT drift to random sites. Being ON a results page that lists the requested kind of data is SUCCESS, not a block — extract the visible rows and output "complete". NEVER output "blocked" or say you found nothing WHILE result rows are visible in the snapshot. If the list looks empty, FIRST "wait" (timeoutMs ~3000) or "scroll" (results load lazily or sit below the fold) and re-observe before concluding anything.
...
Action kinds (the "kind" is exactly the word before the parenthesis): navigate(url) · click(ref) · type(ref,text) · select(ref,values) for a native <select> · fill(fields:[{{ref,type,value}}]) · press(key) · scroll(direction,amount) · hover(ref) · scrollIntoView(ref) · wait(text,timeoutMs)
...
fn validate_browser_loop_action(
    ...
    match kind {
        ...
        "navigate" => {
            ...
            if url.is_empty() {
                return Err(BrowserAutomationError::InvalidResponse(
                    "browser loop navigate action missing url".to_string(),
                ));
            }
            // Only real web navigation — never javascript:/data: schemes (those
            // are a script-execution vector the action gate must not allow).
            if !(url.starts_with("http://") || url.starts_with("https://")) {
                return Err(BrowserAutomationError::InvalidResponse(format!(
                    "browser loop navigate url must be http(s): {url}"
                )));
            }
            Ok(())
        }
