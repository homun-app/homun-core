...
//!
//! WHY: the P0 `gateway.log` has the raw `[plan]`/`[answer]` eprintlns and `crate::trace`
//! (`tool_trace_dump`) is a HASHED parity oracle (ADR 0024) — neither lets a human read "what happened
//! in this turn". This writes ONE readable JSON line per event, turn-scoped, so a bad turn (e.g.
//! delivered no table and claimed it did) can be understood by tailing one file. Local-only, content
//! truncated, best-effort. Kept SEPARATE from `crate::trace` (one responsibility per unit — caposaldo #5).
//!
//! Ported from the pre-ADR-0024 gateway (`feat/piano-ui-completion`) into the engine crate: its pure
//! helpers (`answer_signals`/`has_markdown_table`/`count_sources`/`derive_flags`) and its file sink are
//! engine-appropriate, and the loop that records the in-turn events now lives here (`agent_loop`). It is
//! a pure OBSERVABILITY sink threaded through `run_turn` — it records what the turn did, and NEVER
...
pub enum TurnEvent {
    /// Recorded at the ABSOLUTE ENTRY of the turn — before memory recall, prompt-build, session setup
    /// or the model call. If a `turn_received` has NO matching `turn_start`, the turn HUNG IN SETUP
    /// (the 2026-07-04 minimax case): recall / prompt-build / browser-session setup blocked before
    /// generation ever started. This is the ONLY event that makes a pre-generation hang visible —
    /// `turn_start` is emitted only after setup completes, so on its own it can't reveal a setup stall.
    TurnReceived {
        prompt_head: String,
        prompt_len: usize,
        mode: String,
...
/// it can be threaded into `run_turn` and passed to the tool chokepoint without guarding call sites.
#[derive(Clone)]
...
/// interrupt a turn (mirror `crate::trace::append`).
...
mod tests {
    ...
    #[test]
    fn turn_received_serializes_with_its_kind_tag() {
        // The setup-hang sentinel must be greppable by `kind` and never panic on a bad dir.
        let ev = TurnEvent::TurnReceived {
            prompt_head: "cerca notizie".into(),
            prompt_len: 13,
            mode: "agent".into(),
            model: "minimax-m3:cloud".into(),
        };
