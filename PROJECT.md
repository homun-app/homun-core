...
- Command `submit_user_prompt` collegato al composer: il prompt entra nel Tauri Core, non viene salvato come raw payload nel read model, viene compreso tramite `PromptBrain`/Gemma locale con JSON validato e solo dopo la route strutturata esegue azioni locali come ora/data o calcolo.
