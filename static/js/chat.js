...
function updateReasoningCount() {
    if (reasoningSectionEl) {
        ...
        if (labelEl) {
            labelEl.textContent = window.HomunChatTools.reasoningHeadline(reasoningTools);
        }
    }
}
...
function addReasoningNote(text) {
    ...
    reasoningContentEl.appendChild(window.HomunChatTools.buildReasoningNote(text));
}
...
function showToolIndicator(toolName, toolCallData) {
    ...
    toolIndicatorEl.textContent = window.HomunChatTools.toolStatusLabel(
...
function addToolCallCard(toolCallData) {
    ...
    const card = window.HomunChatTools.buildToolCallCard(toolCallData);
...
function endToolIndicator(toolName, toolCallData) {
    ...
    if (activeTools.length > 0 && toolIndicatorEl) {
        ...
        toolIndicatorEl.textContent = window.HomunChatTools.toolStatusLabel(activeTools[activeTools.length - 1]);
...
function createMessageActions(role, msgDiv) {
    return window.HomunChatTools.createMessageActions(role, msgDiv, {
        startEditMessage,
    });
}
...
function addMessage(role, content, toolsUsed, options = {}) {
    ...
    if (toolsUsed && toolsUsed.length > 0) {
        const section = window.HomunChatTools.buildHistoricalToolsSection(toolsUsed);
        div.prepend(section);
    }
