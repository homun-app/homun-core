...
const rcBaselinePattern = /base `main` aggiornata a #405 \(`b76fe0d2`\), diff RC locale verificato prima della PR/;
...
test("status doc records a concrete current main baseline without stale slice branch", () => {
  assert.match(
    statusDoc,
    /\| HEAD codice verificato \| base `main` aggiornata a #[0-9]+ \(`[0-9a-f]{8}`\), diff RC locale verificato prima della PR \|/,
  );
...
test("status doc records the merged composer-mode presenter cleanup slice", () => {
  ...
  assert.match(statusDoc, /\| Branch \| `fabio\/rc-readiness-2026-08-26` \|/);
...
test("status doc records the local chat fallback empty transcript contract", () => {
  assert.match(statusDoc, /Slice local chat ready seed mergeata #400/);
...
test("status doc records the retired fixed empty hero subtitle contract", () => {
  assert.match(statusDoc, /Slice empty hero subtitle mergeata #401/);
  assert.match(statusDoc, /cataloghi i18n non devono reintrodurre `chat\.emptyHeroSub`/);
  assert.match(statusDoc, /fixed subtitle[\s\S]*ritirato/);
  assert.match(statusDoc, /non copy statico che promette risposte locali/);
});

test("status doc records the thread preview readiness copy cleanup contract", () => {
  assert.match(statusDoc, /Slice thread preview readiness copy mergeata #402/);
  assert.match(statusDoc, /defaultChatThread` e `updateThreadPreview`/);
  assert.match(statusDoc, /Local session ready/);
  assert.match(statusDoc, /Local chat ready/);
  assert.match(statusDoc, /preview sidebar deve arrivare da messaggi\/read model reali/);
});

test("status doc records the retired local session readiness i18n key contract", () => {
  assert.match(statusDoc, /Slice localSessionReady i18n key mergeata #403/);
  assert.match(statusDoc, /chat\.localSessionReady/);
  assert.match(statusDoc, /copy morta di readiness locale/);
});

test("status doc records the chatApi static local subtitle cleanup contract", () => {
  assert.match(statusDoc, /Slice chatApi static local subtitles mergeata #404/);
  assert.match(statusDoc, /chatApi` resta confinato come modalita' offline\/dev/);
  assert.match(statusDoc, /Local chat/);
  assert.match(statusDoc, /Local model/);
  assert.match(statusDoc, /preview deriva dall'ultimo messaggio/);
});

test("status doc records the post chatApi non-browser audit boundary", () => {
